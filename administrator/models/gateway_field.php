 ...
 * Wbty_payments model.
 */
class Wbty_paymentsModelgateway_field extends WbtyModelAdmin
{
	protected $text_prefix = 'com_wbty_payments';
	protected $com_name = 'wbty_payments';
	protected $list_name = 'gateway_fields';

	public function getTable($type = 'gateway_fields', $prefix = 'Wbty_paymentsTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform', $key=0)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_payments.gateway_field.'.$control.'.'.$key, 'gateway_field', array('control' => $control, 'load_data' => $loadData, 'key'=>$key));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_payments.edit.gateway_field.data', array());
	...
	{
		if ($item['gateway_field'] = parent::getItem($pk)) {
	...
	protected function prepareTable(&$table)
	{
		...
		if (JRequest::getVar('gateway_id')) {
	$table->gateway_id=JRequest::getVar('gateway_id');
}
