 ...
 * Wbty_payments model.
 */
class Wbty_paymentsModelgateway_field_value extends WbtyModelAdmin
{
	protected $text_prefix = 'com_wbty_payments';
	protected $com_name = 'wbty_payments';
	protected $list_name = 'gateway_field_values';

	public function getTable($type = 'gateway_field_values', $prefix = 'Wbty_paymentsTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform', $key=0)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_payments.gateway_field_value.'.$control.'.'.$key, 'gateway_field_value', array('control' => $control, 'load_data' => $loadData, 'key'=>$key));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_payments.edit.gateway_field_value.data', array());
	...
	{
		if ($item['gateway_field_value'] = parent::getItem($pk)) {
