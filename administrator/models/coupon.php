 ...
 * Wbty_payments model.
 */
class Wbty_paymentsModelcoupon extends WbtyModelAdmin
{
	protected $text_prefix = 'com_wbty_payments';
	protected $com_name = 'wbty_payments';
	protected $list_name = 'coupons';

	public function getTable($type = 'coupons', $prefix = 'Wbty_paymentsTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform', $key=0)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_payments.coupon.'.$control.'.'.$key, 'coupon', array('control' => $control, 'load_data' => $loadData, 'key'=>$key));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_payments.edit.coupon.data', array());
	...
	{
		if ($item['coupon'] = parent::getItem($pk)) {
