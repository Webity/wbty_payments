 ...
 * Wbty_payments model.
 */
class Wbty_paymentsModelship extends WbtyModelAdmin
{
	protected $text_prefix = 'com_wbty_payments';
	protected $com_name = 'wbty_payments';
	protected $list_name = 'shipping';

	public function getTable($type = 'shipping', $prefix = 'Wbty_paymentsTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform', $key=0)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_payments.ship.'.$control.'.'.$key, 'ship', array('control' => $control, 'load_data' => $loadData, 'key'=>$key));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_payments.edit.ship.data', array());
	...
	{
		if ($item['ship'] = parent::getItem($pk)) {
