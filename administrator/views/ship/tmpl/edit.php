...
window.addEvent('domready', function(){
    ...
    document.shipvalidator = document.formvalidator;
    jQuery('#ship-form .toolbar-list a').each(function() {
        $(this).attr('data-onclick', $(this).attr('onclick')).attr('onclick','');
    });
    jQuery('#ship-form .toolbar-list a').click(function() {
        Joomla.submitbutton = document.shipsubmitbutton;
...
Joomla.submitbutton = function(task)
{
    ...
    if (task == 'ship.cancel' || document.shipvalidator.isValid(document.id('ship-form'))) {
        Joomla.submitform(task, document.getElementById('ship-form'));
    }
    else {
        alert('<?php echo $this->escape(JText::_('JGLOBAL_VALIDATION_FORM_FAILED'));?>');
    }
}
document.shipsubmitbutton = Joomla.submitbutton;
<?php
...
<form action="<?php echo JRoute::_('index.php?option=com_wbty_payments&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="ship-form" class="form-validate form-horizontal">
    <div class="row-fluid">
        <div class="span6">
            <fieldset class="adminform parentform" data-controller="ship" data-task="ship.ajax_save">
                <legend><?php echo JText::_('COM_WBTY_PAYMENTS_LEGEND_SHIP'); ?></legend>
                <div class="items">
                    <?php
                        foreach($this->form->getFieldset('ship') as $field):
                    ...
                    <div class="control-group">
                        <div class="controls">
                            <span class="btn btn-success save-primary"><i class="icon-ok"></i> Save Shipping Info</span>
                        </div>
                    </div>
                </div>
    ...
    <input type="hidden" name="form_name" id="form_name" value="ship" />
